...
int main() {
    vector<vector<double> > eom = eomlee(64, 100, 100, 1000, 500);

    for (int i = 0;i< eom.size();i++) {
        cout << "Interação " << i + 1 << endl;
        cout << "Total slots: " << eom[i][0] << endl;
        cout << "Slots com colisão: " << eom[i][1] << endl;
        cout << "Slots vazios: " << eom[i][2] << endl << endl;
        cout << "Tempo: " << eom[i][3] << endl;
        cout << "Fluxo: " << eom[i][4] << endl << endl;
    }
}